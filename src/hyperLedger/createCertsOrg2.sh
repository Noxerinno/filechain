...
export IP_ORG2=$1
echo "[Step 1] Enroling client and registering peer and user identitities (${IP_ORG2})"

export FABRIC_CA_CLIENT_HOME=$REGISTRAR_DIR

# Enroll client to interact
./bin/fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=org2.example.com -m admin -u http://admin:adminpw@${IP_ORG2}:7054

sleep 10

# Register admin identity and peer
./bin/fabric-ca-client register --id.name Admin@org2.example.com --id.secret mysecret --id.type client --id.affiliation org2 -u http://${IP_ORG2}:7054 

./bin/fabric-ca-client register --id.name peer0.org2.example.com --id.secret mysecret --id.type peer --id.affiliation org2 -u http://${IP_ORG2}:7054

./bin/fabric-ca-client register --id.name orderer0.org2.example.com --id.secret mysecret --id.type orderer --id.affiliation org2 -u http://${IP_ORG2}:7054
...
./bin/fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=org2.example.com -m Admin@org2.example.com -u http://Admin@org2.example.com:mysecret@${IP_ORG2}:7054
...
./bin/fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=org2.example.com -m peer0.org2.example.com -u http://Admin@org2.example.com:mysecret@${IP_ORG2}:7054 
...
./bin/fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=org2.example.com -m orderer.org2.example.com -u http://Admin@org2.example.com:mysecret@${IP_ORG2}:7054 
...
cp -r $PWD/crypto-config/ica-servers/certsICAOrg2/* $ORG_DIR/ca
