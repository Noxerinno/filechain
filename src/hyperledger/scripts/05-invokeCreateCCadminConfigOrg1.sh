...
# 1 argument is required, which is the json containing all the info about the AdminConfig
...
#The form of the json
#json="{\"IpfsId\":\"1\",\"AdminIpAddress\":\"2\",\"SwarmKey\":\"3\",\"ClusterSecret\":\"4\",\"ClusterPeerId\":\"5\"}"
json=$1
#read -p "Press any key to continue (invoke Create) ..."
peer chaincode invoke -o orderer0.org1.example.com:7050 --cafile $ORDERER_CA -C $CHANNEL_NAME -n adminConfig-contract --peerAddresses $CORE_PEER_ADDRESS --cafile $ORG1_CA -c '{"Args":["Create","'$1'"]}' #2>/dev/null
