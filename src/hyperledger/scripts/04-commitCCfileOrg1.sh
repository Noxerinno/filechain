...
peer lifecycle chaincode commit -o orderer0.org1.example.com:7050 --channelID $CHANNEL_NAME --name "file-contract" --version 1.0 --sequence 1 --cafile $ORDERER_CA --peerAddresses peer0.org1.example.com:7051 --peerAddresses peer0.org2.example.com:8051 --signature-policy "OR('Org1MSP.member','Org2MSP.member')" #2>/dev/null
