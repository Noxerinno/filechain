...
<section class="flex w-full h-full transition-colors overflow-x-hidden" ref="drag" :class="isOver ? 'bg-blue-200 shadow-inner' : 'bg-transparent'" v-cloak @drop.prevent="addFile" @dragover.prevent>
	<div v-if="mounted" class="flex w-full h-full">
		<div class="h-full w-full py-7 mx-44 transition duration-500 transform" v-on:click="select(null)" :class="selected ? '-translate-x-44' : '-translate-x-0'">
			...
			<div class="grid grid-cols-5 gap-5 w-full">
				<div class="cursor-pointer m-5 py-5 bg-white transition-color duration-500 rounded-md shadow-lg" v-for="(elm, idx) in orderedFiles" :key="idx" v-on:click.stop v-on:click="select(elm)" :class="selected == elm ? 'bg-blue-200' : ''"> 
		...
		<div class="text-center font-bold absolute w-1/3 h-16 bg-white shadow-lg rounded-lg left-1/2 transition-transform duration-500 transform -translate-x-1/4" :class="done ? 'translate-y-2' : '-translate-y-16'">
			<div class="h-full flex justify-center items-center">
				<font-awesome-icon :icon="['fas', 'check']" size="2x" class="mr-5 text-green-500"/>
...
export default {
	...
	computed: {
		...
		orderedFiles: function () {
			return _.orderBy(this.files, 'timestamp', 'asc')
		}
	},
	methods: {
		async addFile(elm) {
