...
fabric-ca-client register --id.name Admin@org2.example.com --id.secret mysecret --id.type client --id.affiliation org1 -u http://localhost:8054 

fabric-ca-client register --id.name peer0.org2.example.com --id.secret mysecret --id.type peer --id.affiliation org1 -u http://localhost:8054 
...
# Enroll client to interact
fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=org1.example.com -m admin -u http://adminCA:adminpw@localhost:7054 
...
fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=org1.example.com -m Admin@org1.example.com -u http://Admin@org1.example.com:mysecret@localhost:7054 
mkdir -p $ADMIN_DIR/msp/admincerts && cp $ADMIN_DIR/msp/signcerts/*.pem $ADMIN_DIR/msp/admincerts/
...
export FABRIC_CA_CLIENT_HOME=$PEER_DIR
fabric-ca-client enroll --csr.names C=ES,ST=Madrid,L=Madrid,O=org1.example.com -m peer0.org1.example.com -u http://Admin@org1.example.com:mysecret@localhost:7054 
mkdir -p $PEER_DIR/msp/admincerts && cp $ADMIN_DIR/msp/signcerts/*.pem $PEER_DIR/msp/admincerts/
sleep 2
echo "[Step 3] Completed"
